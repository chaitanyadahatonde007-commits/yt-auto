...
async function renderAutopilot() {
  ...
  app.innerHTML = `
    ...
    <div class="studio">
      <form class="panel" id="ap">
        ...
        <p class="notice">${pack.job && pack.busy ? esc(`${pack.job.step || "working"} · ${pack.job.message || ""} · ${Math.round((pack.job.progress || 0) * 100)}%`) : ""}</p>
        <p class="toast ${pack._err ? "err" : ""}" id="msg">${pack._err ? esc(pack._err) : (pack.busy ? "A video is being made now. Leave CMD open." : "")}</p>
      </form>
      <aside class="panel">
  ...
  try {
    const t = await api("/api/trends", { timeout: 4000 });
  ...
  if (window._apTimer) clearTimeout(window._apTimer);
  if (pack.busy || (pack.runs || []).some((r) => r.status === "running" || r.status === "queued")) {
    window._apTimer = setTimeout(() => {
      if (route().name === "autopilot") renderAutopilot();
    }, 2500);
  }
}
...
async function renderSettings() {
  ...
  app.innerHTML = `
    ...
    <form class="panel" id="set" style="max-width:720px">
      ...
      <h2 class="section">Pexels key — real stock video</h2>
      <div class="notice">
        ${settings.has_pexels ? "Pexels is connected." : "From pexels.com/api. Free stock video and photos."}
        <ul>
          ${(settings.pexels_does || ["Pulls real stock video", "Competes with WaveSpeed motion", "Backup stills if AI fails"]).map((item) => `<li>${esc(item)}</li>`).join("")}
        </ul>
